<# Filename: 		Activate-PIMRole.0.5.ps1
    =============================================================================
    .SYNOPSIS
...
Connect-MgGraph -Scopes "RoleManagement.Read.Directory", "RoleManagement.ReadWrite.Directory" -UseDeviceAuthentication -NoWelcome
...
try {
    if (Test-Path $savePath) {
        ...
        foreach ($entry in $historyArray) {
            ...
            $HistoryComboBox.Items.Add($entry.Reason)
        }
    }
}
catch {
...
$HistoryComboBox.Add_SelectionChanged({
        $selectedHistory = $history | Where-Object { $_.Reason -eq $HistoryComboBox.SelectedItem }
        if ($selectedHistory) {
...
$ActivateButton.Add_Click({
        ...
        foreach ($SelectedRole in $SelectedRoles) {
            ...
            # Activate PIM role.
            Write-Verbose -Verbose -Message "Activating PIM role '$($SelectedRole.RoleDisplayName)'..."
            ...
            Write-Verbose -Verbose -Message "$($SelectedRole.RoleDisplayName) has been activated until $($Schedule.Expiration.Duration)!"
        }
