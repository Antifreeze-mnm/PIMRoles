...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="10">
        ...
        <StackPanel>
            ...
            <RadioButton Name="BrowserAuth"
                         Content="Browser Authentication (Default)"
                         IsChecked="True"
                         Margin="0,10,0,5"/>

            <TextBlock Text="Uses your existing browser session."
                       Margin="20,0,0,15"
                       Foreground="Gray"/>

            <RadioButton Name="DeviceCodeAuth"
                         Content="Device Code Authentication"
                         Margin="0,5,0,5"/>

            <TextBlock Text="Prompts you to choose the correct account and tenant."
                       Margin="20,0,0,15"
                       Foreground="Gray"/>
...
"@
    ...
    $BrowserAuth = $window.FindName("BrowserAuth")
    $DeviceCodeAuth = $window.FindName("DeviceCodeAuth")
    $OkButton = $window.FindName("OkButton")
    $CancelButton = $window.FindName("CancelButton")

    $script:AuthMethod = "Browser"

    $OkButton.Add_Click({

            if ($DeviceCodeAuth.IsChecked) {
...
$AuthenticationMethod = Select-AuthenticationMethod

if (-not $AuthenticationMethod) {
    return
}
...
try {
    ...
    if (-not $mgContext) {
        Write-Host "Connecting to Microsoft Graph for PIM role management..." -ForegroundColor Yellow
        $GraphScopes = @(
    ...
    # Get the authentication context again after ensuring connection
...
"@
    ...
    if ($Result -ne [System.Windows.MessageBoxResult]::Yes) {
        ...
        Write-Host "User cancelled after tenant verification." -ForegroundColor Yellow
    ...
    # Get the User Principal Name (UPN) from the context. This is reliable.
    $currentUserUPN = $mgContext.Account

    # Use the UPN with Get-MgUser to get the full user object. This avoids Get-MgMe.
    $CurrentUser = Get-MgUser -UserId $currentUserUPN -ErrorAction Stop
    $CurrentAccountId = $CurrentUser.Id
    
    Write-Host "Successfully connected and identified as $($CurrentUser.DisplayName) ($($CurrentUser.UserPrincipalName))" -ForegroundColor Green


    
}
catch {
    Write-Error "Failed to authenticate or identify the current user. Please ensure the Microsoft.Graph.Users and Microsoft.Graph.Authentication modules are installed correctly. Error: $_"
    # Close the loading window and exit if authentication fails
    $loadingWindow.Close()
    if ($Host.Name -eq "ConsoleHost") { Read-Host "Press Enter to exit" }
    exit
}
# --- END: Integrated Authentication ---
